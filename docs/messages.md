...
- Receipts follow your account's **read-receipts privacy setting** (WhatsApp → Settings → Privacy → Read receipts). When it is off, a `read-self` receipt is sent instead of `read`, in direct and group chats alike: it only syncs the read state to your own devices, and the sender does not see blue ticks. The result reports which one was sent (`receipt`: `read` or `read-self`; `sender_notified` is `true` only for `read`), and the human output prints a note for `read-self`. The setting itself is never changed.
- Receipts apply to messages you received: any `--id` the local store knows as one of your own sent messages is rejected. IDs that are not in the store are accepted in direct chats and, with `--sender`, in groups, so an unsynced received message can still be marked read.
